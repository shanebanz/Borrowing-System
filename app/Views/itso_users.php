...
<head>
    ...
    <title>User Management - ITSO</title>
...
<div class="container">
    ...
    <aside class="sidebar">
        ...
        <nav class="menu">
            ...
            <a href="<?= site_url('itso/users') ?>" class="menu-item active">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    if (!empty($user['profile_image'])) {
                                        $filePath = WRITEPATH . 'uploads/thumbnails/profiles/' . $user['profile_image'];
                                        if (file_exists($filePath)) {
                                            $hasProfileImage = true;
                                            $imageUrl = base_url('media/thumbnails/profiles/' . $user['profile_image']);
                                        }
                                    }
                                    
                                    if ($hasProfileImage): ?>
                                        <img src="<?= $imageUrl ?>" alt="Profile" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2px solid #ddd;">
                                    <?php else: ?>
                                        <div class="no-profile-pic">NO<br>PHOTO</div>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td>
                                    <div class="action-btns">
                                        ...
                                        <?php if($user['is_active'] == 1): ?>
                                            <form action="<?= site_url('itso/users/deactivate') ?>" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to deactivate this user?')">
                                        ...
                                        <?php else: ?>
                                            <form action="<?= site_url('itso/users/activate') ?>" method="POST" style="display: inline;">
                                                <input type="hidden" name="user_id" value="<?= esc($user['user_id']) ?>">
...
<div id="editModal" class="modal">
    <div class="modal-content">
        ...
        <form id="editForm" action="<?= site_url('itso/users/update') ?>" method="POST" enctype="multipart/form-data">
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="file" id="profile_image" name="profile_image" accept="image/*" style="padding: 5px;">
...
<script>
    function openEditModal(userId) {
        fetch(`<?= site_url('itso/users/get/') ?>${userId}`)
